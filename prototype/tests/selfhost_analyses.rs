...
//!
//! Harness shape reuses slice 3: embed the fixture source as a `[N]u8` literal,
//! lex via lexer.cnr, parse+analyze+dump via analyses.cnr, reconstruct the dump
//! from `Run.trace`, compare to the filtered+sorted oracle rendering. The
//! generated `main` is `alloc` because the analysis's growable diagnostic buffer
//! is a `Vec` (its first self-hosting customer).
