//! Vendored subset of the `linked_list_allocator` crate: the first-fit
//! hole-list free-list engine actually exercised by this baseline.
//!
//! # Provenance
//!
...
//! the comparison target (a port in a language with no crate ecosystem) is
//! measured against the same in-source machinery, not against code hidden in a
//! cargo dependency.
//!
//! - **Crate:** `linked_list_allocator`, version `0.10.6` (crates.io).
//! - **Upstream:** <https://github.com/rust-osdev/linked-list-allocator>.
//! - **Commit:** `a1d90498ff82872c5eb89edef77a08b25b4f14c4`.
...
//! # What is vendored
//!
//! The used subset only: the [`Heap`] façade (`src/lib.rs`) plus the
//! [`HoleList`] engine (`src/hole.rs`) — first-fit placement, alignment-aware
//! splitting, and adjacent-hole coalescing on free. Unused crate surface
//! (`init`/`extend`/`*_from_slice`, `LockedHeap`, `GlobalAlloc`/`Allocator`
//! impls, spin support, the `pending_extend` reclaim state that only `extend`
//! consumes, debug/fuzzing hooks, and the upstream test module) is removed as
//! dead code per the ruling. The retained code is upstream's, unchanged except
//! for that dead-code removal and the minimal edits required to compile it in
//! this module; its idiomaticity is upstream's, which is the point.
...
impl Heap {
    ...
    ///
    /// The `heap_bottom` pointer is automatically aligned, so the bottom of the
    /// heap might be larger than `heap_bottom` after construction.
    ///
    /// # Safety
    ///
    ...
    /// # Safety
    ///
    /// `ptr` must be a pointer returned by a call to the
    /// [`allocate_first_fit`][Self::allocate_first_fit] function with identical
    /// layout. Undefined behavior may occur for invalid arguments.
...
impl HoleList {
    ...
    ///
    /// `ptr` must be a pointer returned by a call to the
    /// [`allocate_first_fit`][HoleList::allocate_first_fit] function with
