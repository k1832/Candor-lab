//! In-place `sort` over `Vec[i64]` by a first-class comparator. Insertion
...
//!
//! NOTE: sort is monomorphic (`Vec[i64]`) rather than generic `sort[T]` because a
//! generic `Vec[T]` parameter in a generic fn signature is currently rejected
//! with E1004 (`resolve_gty` in src/generics.rs doesn't treat compiler-known
//! `Vec`/`Map` as generic constructors the way monomorphization's `rewrite_ty`
//! does). See the run report.
...
fn sort(v: write Vec[i64], less: fn(read i64, read i64) -> bool) alloc -> unit {
    let n: usize = len(read v.*);
    let mut i: usize = 1usize;
    while i < n {
        let key: i64 = get(read v.*, i).*;
        let mut j: usize = i;
        while j > 0usize {
