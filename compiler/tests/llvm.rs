...
// compiled -O2 process must die with the interpreters' clean bad_pointer fault
// at span 0 (exit 2 + fault JSON) — never a segfault. Native-only pin: the
...
// not grow the bump under `clang -O2` — the frame-local address of two
// identical calls is the same (per-call growth == 0, the memo §1 shapes),
// matching the reclaiming MIR interpreter. Native-only pin on the address
// deltas (engine-internal values).
#[test]
fn gate_llvm_stack_bump_parity_flat_calls() {
    assert!(clang_available(), "clang unavailable");
    let src = "struct S { a: i64, b: i64, c: i64 } struct D { v: i64 } drop(write self) { } \
        ...
        fn main() -> i64 { \
            ...
            return (s2.a - s1.a) - 1; }";
    let srcpath = std::env::temp_dir().join(format!("candor-llvm-src-{}-bumppar.cnr", std::process::id()));
    std::fs::write(&srcpath, src).unwrap();
    let a = llvm_outcome(&srcpath, "bumppar").expect("compile+run should succeed");
    let _ = std::fs::remove_file(&srcpath);
