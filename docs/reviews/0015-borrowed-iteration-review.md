...
**Disposition:** _(pending deciding authority)_ — **checker fix landed 2026-07-13** (implements suggested direction (1), the cross-cutting loan-propagation repair; ledger `LOAN-COPY-UAF`). A `read`/`write` borrow copied into a binding now propagates its source loan(s) to the new binding's live range (`propagate_place_loans` + the extended `carries_borrow` gate, `src/check/{mod,expr,stmt}.rs`), so the verified repro is rejected E0802 and the copy-then-{move,write,return} family is caught (8 regression tests in `tests/loans.rs`), full suite + selfhost loans oracle green, no false positives. The UAF is closed independent of 0015; whether 0015 additionally adopts the (2) non-escape constraint on `x`, and the acceptance of 0015 itself, remain the deciding authority's call. Residual — NOW ALSO CLOSED (2026-07-13): the analogous `slice`-copy shed (a `copy` `Type::Slice`/`SliceMut` aliasing its backing) was verified with its own repro (`docs/reviews/0015-slice-copy-uaf-repro.cnr`: `slice_of(bx.*)`, copy, then `unbox` — check-clean + tree-oracle fault pre-fix) and closed by extending the same `propagate_place_loans` + `carries_borrow` gate to `Slice`/`SliceMut`; four `slice_copy_*` regressions added, full suite 703 green incl. the slice corpus, no verdict changes — see ledger. Second residual — ALSO NOW CLOSED (2026-07-13, ledger `STR-VIEW-UAF`): the `str`/`[u8]` view retypes (`as_str`/`as_bytes`/`substr`/`str_from_unchecked`) recorded NO loan on the source `String` at all (a distinct sub-class — untracked provenance, not a copy shed). VERIFIED repro: `let vb = as_bytes(as_str(read s));` then an `append` that forces a `string_reserve` GROWTH (free-old) while `vb` is live — check-clean pre-fix, and a stale-read on every engine (`vb[0]` 97 -> 0 as the freed buffer took the free-list header; return 97000 vs a sound 97097 on tree-walker, MIR, and native). Closed by recognizing the four retypes + a bare `str` place in `carries_borrow` and extending `propagate_place_loans` to `Type::Str`, so the view carries the source loan for its live range (NLL releases it at the view's last use — make-view-use-done unaffected). The repro fails check E0801; four `str_view_*`/`substr_view_*` regressions added; full suite 707 green incl. the heavy as_bytes/as_str corpus + selfhost + native, no verdict changes.
