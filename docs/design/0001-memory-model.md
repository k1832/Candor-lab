...
**Revision note (2026-07-08).** §2.1 and §4.2 amended to agree with accepted designs 0004 (`field_ptr` — §4.2 safe carve-out + E0510) and 0005 (implicit call-site reborrow supersedes the 2026-07-06 explicit-reborrow clarification), per `docs/reviews/2026-07-08-design-0004-0005-review-1.md`.

**Revision note (2026-07-08).** §3.3 gained a forward-note on region-parameter declaration under generics, per joint adversarial review #1 of designs 0007/0008 (`docs/reviews/2026-07-08-design-0007-0008-review-1.md`).
...
- A type may declare **at most one** `drop(write self) { ... }` hook. It runs **before** the value's fields are destroyed. It may not move the **value** out of `self` or any field out of it via a deref-path move (that is E0310) — `self` is a `write` borrow, so moving `self` itself moves only the borrow handle, which is ordinary and harmless (the fields it points to still destruct exactly once, since the handle carries no ownership; retest 2026-07-08, `docs/reviews/2026-07-08-e0809-retest-2.md`). It runs exactly once per live value. **The hook body is ordinary checked code** — resolved, type-, dataflow-, and loan-checked as a synthetic `fn drop(self: write StructT) -> unit`, so every rule of this document (a field move out via `deref self` is E0310, a write through a shared borrow is E0809, an unknown name is a resolution error) holds inside it, and if that body is **`alloc`-effecting** (it allocates, boxes, or drops a box-bearing local) the *type* becomes **alloc-on-drop**: every scheduled drop of it propagates the `alloc` effect to the enclosing function exactly like a `Box` field (§6.3). (Retest 2026-07-08; `docs/reviews/2026-07-08-e0809-retest-1.md`.)
...
3. **No conditional initialization divergence at a drop point.** The dual of rule 1 for the *initialization* dimension, and for the same reason. At a place's drop point (scope exit — including one reached via `return`/`break`/`continue`), a place whose type is **needs-drop** — it has a `drop` hook, or transitively contains a drop-hooked type or a `Box` — must have the **same initialization state** on all incoming paths: initialized on all, or uninitialized/moved on all. A place initialized on one branch and not another, still live at its scope exit, would leave the interpreter to decide the drop from a runtime flag — exactly what rule 1 removes for moves. It is rejected (checker error **E0309**); the fix is to initialize on all paths, consume on all paths, or narrow the scope. Drop-inert types (scalars, copy aggregates, `rawptr`, and aggregates of them) are **exempt** — their drop is a no-op, so a path-dependent init state at exit costs nothing and stays legal. (Added per the 2026-07-07 drop-flag finding, which showed rule 1 discharged the static-drop claim only for the move dimension; see §7.4 and 0003 §0.)
...
**Superseded by design 0005 (2026-07-08; `docs/reviews/2026-07-08-design-0004-0005-review-1.md`).** The recorded revisit named just above has fired: the basket's 296 reborrow sites triggered the P13-vs-P2 trade in the open, and design 0005 decides it. **A bare held borrow passed to a `read`/`write`-mode parameter now reborrows** (it does not move); `write`-param + exclusive source yields an exclusive reborrow, `read`-param + exclusive-or-shared source a shared one, and `write`-param + shared source is still the unchanged "cannot reborrow exclusive from shared" error. The explicit `read (deref b)` / `write (deref b)` spelling is retained as accepted input but canonical-formatter-normalized to the bare form (a real-toolchain P16 obligation; the prototype has no formatter). The 2026-07-06 clarification text is kept visible per ledger discipline, not deleted: where it reads "there is no implicit call-site reborrow in the prototype," read "superseded by 0005." Fresh borrows of owned storage keep their keyword (`f(write x)`) unchanged. See 0005 for the decision and its reclassification record.
...
- **Slices count as borrow parameters (OBL-SLICE-REGION).** A slice-typed parameter *is* a borrow, so it counts toward the one-vs-many borrow-parameter test above — this is the rule the checker already implements (E0807 fires when a slice parameter pushes the count to two-or-more). A region variable must therefore be spellable *on* a slice type: the real syntax is `read[r] [T]` / `write[r] [T]` (design 0006 §2.2), and the prototype uses the stopgap `slice[r] T` / `slice_mut[r] T`, so a function with a slice plus another borrow parameter returning a borrow is writable.
...
Operations (all require `unsafe`, with the safe carve-outs noted inline: `offsetof` — like `is_null` — is a **safe** compile-time constant query; it confers no ability to act on memory, and the unsafety lives in the pointer arithmetic that consumes it (clarification 2026-07-06, surfaced by the Stage 2 checker). `field_ptr` (design 0004) is likewise **safe**: it computes a field's address by static offset, gives the pointer no meaning, and is well-formed only when `p : rawptr StructT` and `f` is a statically known field of `StructT` — otherwise checker error **E0510**; it joins `offsetof`/`is_null` on the safe side and is *not* E0501-gated):
...
| `field_ptr(p, f)` | `p: rawptr StructT, f: field of StructT` → `rawptr FieldT` | (**safe**, design 0004) field address `address(p) + offsetof(StructT, f)`; no deref, no null check; ill-formed unless `f` is a static field of `StructT` (**E0510**) |
...
**Dropping or `unbox`ing a `Box` is allocator work, so it is `alloc`-effecting (§6.3).** Calling `free` is as much the allocator's business as calling `alloc`: in an interrupt or other non-`alloc` context it is equally forbidden, and §6.1 already types the vtable's `free` field `alloc`-marked. Therefore `unbox`, and **any scope that drops a `Box` (or a value that transitively owns one)** — a scope-exit drop of a needs-drop box-bearing local, a reassignment that drops such a place, or a box-bearing temporary dying at statement end — makes the enclosing function `alloc`-marked. The honest consequence, stated plainly: **a function that receives a `Box` by value and lets it die is `alloc`-marked** (a function that instead *moves it out* — returns it or passes it on — is not; it frees nothing). This mirrors no_std reality, where an allocation-free routine may neither allocate nor free. (Recorded per the 2026-07-07 soundness review #1, finding 4; `docs/reviews/2026-07-07-soundness-review-1.md`. **Rejected alternative:** exempting `free` from the effect — rejected because it would let "allocation-free" code call into the allocator through the back door of a drop.)
...
- Any call to `box`, any `clone` of a box-bearing type, any call to an `alloc`-marked function, **any `unbox`, and any drop of a `Box`-bearing value scheduled in the body** (a scope-exit drop of a needs-drop box-bearing local, a reassignment-drop of one, a box-bearing temporary dropped at statement end, or a function-exit drop of an owned box-bearing parameter that is still live) makes the enclosing function `alloc`-marked — the *free* side of the effect (§6.2). An **indirect call takes its effect from the fn-pointer's type** (§6.1): calling through an `alloc`-typed pointer — such as any `AllocVtable` field — makes the caller `alloc`. There is **no** vtable-specific rule; the field types carry the effect. A type whose `drop` **hook body** is itself `alloc`-effecting is **alloc-on-drop** (§1.5); a scheduled drop of such a type — or of any value transitively owning one — propagates the `alloc` requirement to the enclosing function exactly like a `Box`, via the same `box_subpaths`/drop-site machinery (retest 2026-07-08).
...
- **Contract clauses are read-only** (soundness review #3, 2026-07-07). Inside a `requires`, `ensures`, or `assert` condition, moving a non-`copy` value, taking a `write`-borrow, passing an `out` argument, or calling a function that takes any argument by `take` (non-copy), `write`, or `out` is a **checker error (E0708)**: under P8 a contract is a check and an oracle, and a check that consumes or mutates the state it inspects is incoherent. Reads, `read`-borrows, and copy-`take` calls are fine.
- **`ensures` accesses are dataflow-checked against the post-body state at each normal return** (soundness review #3). The clause's reads/borrows are analyzed as if written at each return point, over the move/init state the body leaves there — so a clause that dereferences a `Box` the body already `unbox`-ed, or reads a param the body moved, is the ordinary **use-of-moved E0301** (noting the contract position), not an unchecked use-after-free. The clause is analyzed once per return block against that block's own state (not a meet of the return states): this is the precise must-semantics, since the clause is evaluated at runtime at each return. `requires` stays entry-checked and gains the same read-only rule.
...
- **Top-level `static NAME: Type = Expr;`** (grammar 0002 §item): a program-lifetime value for vtables and constants (the `POOL_VTABLE`/`MY_VTABLE` of §6.1/§11.1). **Statics are immutable** (soundness review #3, 2026-07-07): assigning to a static, `write`-borrowing one, or passing one as `out` is a **checker error (E0311)**; reading and `read`-borrowing are legal. A *mutable* global raises real questions (concurrency, aliasing, P9) the single-threaded prototype must not answer by accident — it is a recorded future design question, not an oversight; a basket that needs one returns as a deliberate design change.
...
- **Owned scrutinee.** Each *named* payload is taken **by the value gear**: it **moves** out of the payload sub-place, or **copies** if the payload type is `copy`. A move binding **partial-moves** the scrutinee under §1.6 — so it is rejected when the scrutinee's type has a `drop` hook (no partial move out of a `drop`-hooked type; in the prototype this clause is vacuous for `match`, because the grammar attaches `drop` hooks to structs only — 0002 §2.1, verification #1 finding D-B — and is stated so the §1.6 restriction is already in force if enum hooks are ever added), and the resulting move state must **agree at every control-flow join** (§1.6, rule 1). Payloads a pattern does not name are left owned by the scrutinee and dropped with it; a discriminant-only match that binds nothing does not move the scrutinee.
