...
- A type may declare **at most one** `drop(write self) { ... }` hook. It runs **before** the value's fields are destroyed. It may not move `self` or any field out (it borrows `self` exclusively). It runs exactly once per live value. **The hook body is ordinary checked code** — resolved, type-, dataflow-, and loan-checked as a synthetic `fn drop(self: write StructT) -> unit`, so every rule of this document (a field move out via `deref self` is E0310, a write through a shared borrow is E0809, an unknown name is a resolution error) holds inside it, and if that body is **`alloc`-effecting** (it allocates, boxes, or drops a box-bearing local) the *type* becomes **alloc-on-drop**: every scheduled drop of it propagates the `alloc` effect to the enclosing function exactly like a `Box` field (§6.3). (Retest 2026-07-08; `docs/reviews/2026-07-08-e0809-retest-1.md`.)
