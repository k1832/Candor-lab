...
**RE-REVIEW ACCEPT-INVALID FOUND, THEN RESOLVED (2026-07-08; `docs/reviews/2026-07-08-0003-rereview-impl-verification.md`).** The fresh-session re-review scheduled by the tripwire above, discharging its obligation against the *real binary*, found an accept-invalid — and stated the attribution fairly: the hole is **pre-existing** (confirmed by rebuilding commit b838b62; same repro, zero new-feature syntax), latent through **all four** prior soundness reviews. It refutes claim (b) — the §2.2 XOR aliasing rule — not merely an auxiliary claim: **a write through a SHARED borrow was accepted**, so an exclusive-write and a live shared borrow of the same place coexist. `check_place`'s `deref` arm peeled `Borrow` and `BorrowMut` identically with no mutability check, and `canonical()` collapsed the `deref` to the borrow binding, so the write never conflicted with the shared loan. Design 0001 §2.1 always said a deref-write requires an exclusive borrow; the checker never enforced it. Consequences: exclusive reborrow from a shared borrow (`write (deref b)`) was also accepted, and 0005's desugaring had no shareability gate, so bare `f(b)` with a shared `b` to a write-mode parameter slipped through keywordless — so §2.2's desugaring bullet and 0005 rule 1 *falsely* claimed the gate existed. **RESOLVED in the strict direction** by the deciding authority with a new checker error **E0809**, rejecting (a) any assignment/write through a place whose projection path `deref`s a shared borrow, (b) `write (deref b)` (exclusive reborrow from a shared borrow), and (c) a bare shared borrow to a write-mode parameter (the 0005 shareability gate — an error, **not** desugared). The rule as implemented: **every `deref` on the path to a written place must peel an exclusive (`write`) borrow** (or a `Box`), never a shared one — including a shared `deref` reached via autoderef (`b.f = v`) or a shared `deref` nested under an exclusive one. Writes through exclusive borrows, shared reads, shared reborrows of shared, and shared-downgrade-from-exclusive stay legal. `check/expr.rs` (`reject_write_through_shared`, its non-emitting `write_path_probe`, and the `check_arg_mode` write-mode gate) and `check/stmt.rs` (the `Assign` target) carry the check; the reviewer's `xor1`/`xor2` and each of (a)/(b)/(c) with positive controls are regression tests (`tests/check.rs`). **Measurement-impact check (mandatory disposition 3):** under the fixed checker all ten basket fixtures re-`check` with zero diagnostics and every runnable one runs, and all five scored ports re-`check` clean — the artifacts never exercised the hole, so the frozen measurements stand as a soundness-precondition confirmation. The 'how four reviews missed it' lesson is recorded in §4. A fresh-session retest re-verifies this fix before the scheduler re-port, the standing rule above.
...
- **Loan creation and anchoring (`check/mod.rs`, `check/expr.rs`, `check/stmt.rs`).** Every borrow
  ...
  - **Implicit-reborrow desugaring (design 0005).** When an argument is a **place that already
    ...
    (`f(write x)`) are unaffected — they still wear the keyword. When shareability does **not** admit the mode — a bare **shared** (`read`) borrow filling a **write**-mode parameter — the reborrow is **not** desugared; it is the exclusive-reborrow-from-shared error **E0809** (the design 0005 gate, now real), the same code the direct deref-write and the explicit `write (deref b)` reborrow-from-shared cases carry, discharging §2.1's rule that every `deref` on the path to a written place must peel an exclusive borrow (§0, 2026-07-08).
...
- **Seam-hunting is not a substitute for per-analysis obligation enumeration (2026-07-08 re-review lesson).** The write-through-shared-borrow accept-invalid (§0, E0809) was pre-existing and stayed latent through four prior soundness reviews because every attack targeted interaction *seams* (contracts × dataflow, return-extension × `match`, the E0510 boundary) and conflict-pair *shapes* (two exclusives; shared+exclusive via separate borrow expressions) — the recorded review pattern — while nobody wrote the two-line direct mutation through a shared borrow. The lesson, now recorded: seam-hunting must be paired with enumerating each analysis's **own** obligations line-by-line against the model doc (here 0001 §2.1's "deref-write requires an exclusive borrow"); a rule the model states but the checker never implements is invisible to a seam-only attack, and no amount of seam coverage substitutes for that per-analysis audit.
